...
# Generate all possible chest selections
all_chests = list(range(1, TOTAL_CHESTS + 1))
golden_chests = random.sample(all_chests, GOLDEN_CHESTS)

def calculate_probability(selected_chests):
    """Calculate the probability-based score based on selected chests."""
    gold_found = sum(1 for chest in selected_chests if chest in golden_chests)
...
# Player Selection
selected_chests = st.multiselect("Choose 3 chests:", all_chests, default=[], max_selections=3)
...
if len(selected_chests) == 3:
    ...
    st.write("💰 Gold was hidden in chests:", golden_chests)
